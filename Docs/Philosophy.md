...
through that lens: merely "fast enough" loses to the commercial heavyweights,
and the engine competes on frame time.
...
This is why the flagship stress test is 100,000 flocking agents, each a real
entity with a rendered surface, simulated and drawn **live inside the editor**,
through a 3D camera and a 2D wireframe view at once, not a tech demo running in
isolation. See [The Road to 100k Boids](Performance_100kBoids.md).
...
That regularity does more than keep things tidy: it makes the codebase legible
to a newcomer, safe to refactor in bulk, and buildable in configurations,
because packages declare their own dependencies and resources, so the engine
can be assembled from just the packages a given product needs.
...
Public APIs hand out opaque, type-safe **handles** (small integer tokens) and
free functions that operate on them, not C++ objects.

```cpp
// Illustrative shape of the pattern, not a literal API -- surfaces are
// actually created as entity rows, not through a single free function like
// this. The point holds regardless: an opaque token, and functions that take
// it, instead of a caller holding a Surface* with vtable, lifetime, and ABI
// baggage.
Handle h = Create(desc);
SetMaterialName(h, "pbr/test/iron_smooth");
const HandleArray& all = GetAll();
```
...
The habit has a sharper edge now, and it came out of reading a month of defect
fixes back. **A green check is not evidence on its own.** Two shapes account for
a large share of what got through: a path that could not do its work, took a safe
early out and reported success, and a check that passed because its input was
empty rather than because the thing under test was correct. Both are invisible,
and the second is the worse one, because it reads as coverage.

So the engine counts both. A degrade names itself where it happens and fails a
gate unless someone declared it tolerable. A witness proves a path ran, and a run
that expected one and did not get it fails. Both print at process exit, and every
gate reads the same two numbers. See
[Core](Core.md#three-primitives-a-defect-audit-paid-for).
...
[Materials](Materials.md), where a material is a Lua program with inline shaders.
