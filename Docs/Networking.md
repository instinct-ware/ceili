...
The apply path is deliberately ordinary. A remote peer's edit is a `meta::Write`,
so it updates the row, refreshes the grid, and propagates to whatever else watches
that field, exactly as a local edit would.
...
Play runs the deterministic boids simulation in lockstep on both peers. It shows
the page's point in under a minute: the same session object that mirrors an editor
edit also keeps a live simulation in sync.*
...
does not fork the editor's understanding of an entity. Where an incumbent would
build two subsystems with separate schemas, Ceili has one subsystem with two entry
points.
