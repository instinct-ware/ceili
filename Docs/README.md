...
| [Rendering & Visibility](Rendering.md) *(draft)* | Pluggable render and visibility backends behind one device interface (D3D12 and Vulkan ship today, more in progress), how surfaces get culled and drawn, and the frame clock and record-time memory ring the trunk owns. |
| [Lighting](Lighting.md) *(draft)* | Three lighting paths in one frame: per-light forward, a clustered light grid, and a deferred G-buffer. The measured reason each exists, and the parity gates that hold them together. |
| [Shadows](Shadows.md) *(draft)* | Cascaded directional shadow maps: a per-scene 2x2 atlas, the cascade fit and its quantisation, the multi-frustum cull, the bake siblings, two bias mechanisms, and a cache measured at a twelfth of the bake cost. |
| [Atmosphere](Atmosphere.md) *(draft)* | A physical sky in real units: Hillaire's model, transmittance and multiple-scattering tables baked only when the air changes, a froxel volume that hazes the world with the sky's own integrator, a local dust layer, the time-of-day sun, and the horizon seal. |
| [Meshes & Animation](MeshesAndAnimation.md) *(draft)* | glTF and legacy meshes, one entity with many drawable items, skeletons and clips, joint masks, an animation state machine authored as an asset, root motion, and skinning. |
...
| Document | What it covers |
|----------|----------------|
| [Profiling](Profiling.md) *(draft)* | Two clocks kept apart: the CPU zone tree over per-thread event queues, the GPU timestamp registry whose readback lags by frames in flight, the interleaved A/B harness that makes a comparison trustworthy, and the memory panel's scope, heap, container and GPU-arena views. |
...
| [AI-Assisted Engine Development](AI_Assisted_Engine_Development.md) | A case study of building a serious C++ codebase with Claude Code: conventions, autonomy, and what actually moved the needle. |
