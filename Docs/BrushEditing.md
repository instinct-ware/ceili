...
The core idea: **planes are authoritative and everything else is derived**, so the
editor never maintains topology, and the whole system reduces to one expensive
pure function plus the caches that stop you paying for it twice.
...
passed 2048. A fixed epsilon works near the origin and fails once you build far
from it, so the bug stays hidden until someone works at a large offset.
...
## The interactivity cap is a deliberate choice
