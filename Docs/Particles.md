...
particle of an emitter add light to the [deferred G-buffer](Lighting.md):
...
Two other changes went in with it. A particle is now lit by the scene's ambient
rather than drawn fullbright, and a particle can refract, drawn by the pass that
already draws refracting surfaces.
...
Next: [Rendering & Visibility](Rendering.md) for the pass machinery, or
[Lighting](Lighting.md) for the G-buffer a particle light writes into. Back to the
