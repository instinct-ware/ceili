...
(separation, alignment, cohesion). The demanding part is that every boid is a
first-class citizen of the engine, not the rules:
...
problem](Philosophy.md), not the smallest diff. The fix was a different data
structure, not a faster inner loop.
...
static uint32_t buildBins(core::Array<math::Vec4>& Pos, core::Array<core::data::Key>& Keys,
                          graphics::spatial::ISpatialIndex& Grid, ViewT& View,
...
Fifty thousand boids, at twenty frames per second, is the real starting line for
this page. Everything that follows is the story of taking that 50k-at-20fps grid
build to 100k in a dual 2D/3D editor at interactive rates.
...
From 50k-at-20fps to 100k-in-a-dual-view-editor took a sequence of
profiler-guided changes, not one clever trick, each attacking whatever subsystem
the profiler said was now the tallest bar. The honest way to present this is as a
table of **per-change deltas**: each row is the improvement that change bought
*at the time it was made*, against the bottleneck it targeted. Because fixing the
second-tallest bar does nothing until the tallest bar is gone, the rows do not
sum into a single headline number; read together, though, they tell the whole
story.
...
if (structure_matches && cache.structVisibleCount == vis.drawableCount)
{
...
public:
    ...
    {
        // Hand-written, not inherited: an earlier version routed this join
        // through core::data::GetView and measured a regression on a 100k-boid
        // scene, so it stays a bespoke parallel walk over resolved row pointers,
        // change-detected against each row's cachedPoseHash. Change-detection
        // replaces the boids companion's (count, scene, mode) gate generically.
    }
};
```

The `Contract` is still doing real work here: it declares the system's read/write
set to the scheduler, even though the update itself resolves rows by hand rather
than through the contract's auto-generated view. Because the refresh reads
`Position` in the `kPreSim` band, before the flock, it is the flock's
**predecessor**, so the scheduler knows the flock can overlap freely with cull
and draw downstream. That is the
...
- **The deltas in the arc table are per-change, not additive.** Each row is the
  ...
  is gone. The scoreboard above shows the composed end state; the table shows the
  sequence of changes that produced it.
- **The engine has moved since, and these figures have not been re-measured.**
  Cascaded [shadows](Shadows.md) and the [deferred path](Lighting.md) both landed
  after this date and both apply to this scene. The shadow work has its own
  boids measurement on the way: an instanced shadow bake sibling took the scene
  from 5 fps back to its unshadowed rate, because without it a coalesced batch of
  N casters cost N draws per cascade, so 28 batches issued 327,534 draws. The
  scoreboard above stands as what was measured on 2026-07-09 and nothing more.
...
<!-- MEDIA: a before/after comparison -- ideally the O(N^2)-era 8k flock as a
     ...
     shows the arc in one image. -->

---

## What actually did this

This page can read as "a fast flocking demo," but there is almost no
boids-specific performance code in the result. What carried 50k-at-20fps
...
- General [rendering](Rendering.md) machinery (cull, `BuildBatches`, instancing)
  sharpened against this demanding workload.

Every one of those is a Core or Rendering feature that every other scene in the
engine also gets. The flock forced the shared path to be fast rather than getting a private fast
path of its own, and everything downstream inherited the result. The
[central bet](Philosophy.md) pays out here: one engine, no runtime/tools split,
so sharpening the general machinery for a demanding demo sharpens it for
everyone.
