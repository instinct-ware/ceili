...
`ffi.cdef` declaring it plus an `ffi.load` of the engine's combined native library.
Three projections, one underlying symbol.
...
ceili_graphics_material_shader_constants_GetElements(const uint32_t hConstants, uint64_t* Count)
{
...
Notice what the shim absorbs. The handle crosses as a bare `uint32_t` and is
...
public currency: a fixed-width integer and a `T*` cross a language boundary
trivially; a C++ type with a vtable and a growth policy does not.

Every exported package builds into one combined native library, `EngineScriptingC`,
so the C# and Lua projections both load that single library rather than one per
module. The C# projection of the same symbol is a `DllImport` into it, generated
into `Core.cs` / `Graphics.cs`. The handle also crosses as a generated blittable
`Handle` struct wrapping the same `uint32_t`, with implicit conversions, rather
than a bare integer:

```csharp
// A DllImport binding names the same flat C symbol the C writer emitted.
[DllImport("EngineScriptingC", CallingConvention = CallingConvention.Cdecl,
    EntryPoint = "ceili_graphics_material_shader_constants_GetElements", CharSet = CharSet.Ansi, BestFitMapping = false)]
public static unsafe extern Ceili.Graphics.Material.Shader.Constants.Element* GetElements(
    Ceili.Graphics.Material.Shader.Constants.Handle hConstants, ref ulong Count);
```

And the Lua projection declares the identical symbol to LuaJIT's FFI and loads that
same combined library, generated into `graphics.lua` / `core.lua`. There, the
handle is declared through a typedef (`ceili_graphics_material_shader_constants_Handle`,
itself `typedef unsigned int`), not spelled as a raw integer type inline:

```lua
-- The Lua binding: one ffi.cdef for the symbol, one ffi.load for the combined library.
ffi.cdef[[ceili_graphics_material_shader_constants_Element * ceili_graphics_material_shader_constants_GetElements(const ceili_graphics_material_shader_constants_Handle hConstants,  uint64_t* Count);]]
local lib = ffi.load(ceili.sharedLibPath(constants.ceili.prefix .. constants.ceili.scriptingC))
```
...
CE_AGENT_TOOL("Execute a Lua script in the Ceili engine.  The script has access to the full engine API exposed via Lua bindings (ceili.*).  Returns the script's stdout/log output, or an error message on failure.")
inline ConstStr ExecuteLua(CE_AGENT_PARAM("Script", "The Lua script source to execute.") ConstStr Script);
```
...
```json
{
  ...
  "description": "Execute a Lua script in the Ceili engine.  The script has access to the full engine API exposed via Lua bindings (ceili.*).  Returns the script's stdout/log output, or an error message on failure.",
  "inputSchema": {
    "type": "object",
    "properties": { "Script": { "type": "string", "description": "The Lua script source to execute." } },
    "required": ["Script"]
  }
}
```
