...
same assistant, in its coding form, builds the engine. The engine ships the very
model that helped build it. The two halves are the same story told from
...
[trunk-plus-backends pattern](Components.md#the-trunk-and-backends-pattern): a generic
...
> the code, and inconsistent with itself. It lists five backends: `CID_AgentStub`
> ("this package"), `CID_AgentClaude`, `CID_AgentOpenAI` (future), `CID_AgentOllama`
> (future), and `CID_AgentNull` (future). Only two of those are declared component
> IDs at all (`Agent/Module.h` declares just `CID_Agent` and `CID_AgentNull`), and
> the comment mislabels the one that is real: `AgentNull` is the trunk's actual
> fallback implementation (`AgentNull.cpp`), aliased as the default in
> `Agent/ModuleFactories.cpp`, not future work. The two backends that exist and run
> today are `AgentNull` (the fallback, live as the default alias) and
> `AgentClaude`. The generic-CID pattern is what makes adding a third a matter of
> one alias line, not a refactor.

---

## Half one: the engine ships Claude

`AgentClaude` is the backend that turns `sendMessage` into a real conversation.
Rather than link a model or embed an inference runtime, it spawns the `claude`
CLI as a child process and talks to it over pipes. The spawn is an ordinary
`core::process::Desc`:
...
the MCP server exposes is emitted by the *same generator pass* that produces the
C, C#, and Lua bindings, not written by hand (see
...
assistant embedded in the engine is literally the coding assistant that *builds*
the engine, the same tool doing both jobs. Claude Code writes the C++, the Lua,
and the C#; the engine then ships that same model, pointed at its own scripting
API.

That symmetry is a design constraint that paid off, not a marketing flourish. To
embed an agent that can usefully drive the engine, the engine's API has to be:
...
All three are just *good API* features, not agent-specific ones. The
...
[case study](AI_Assisted_Engine_Development.md) covers the workflow, the
...
The embedded agent is the [component system](Components.md) choosing a backend,
the [scripting API](Core.md#strong-types-and-handles) crossing a language
boundary, and the [generator](ScriptGeneration.md) emitting one more artifact
next to the bindings it already produces, not a new subsystem. Count the reuse:
...
[case study](AI_Assisted_Engine_Development.md) for the other half of the
