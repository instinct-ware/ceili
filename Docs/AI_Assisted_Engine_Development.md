...
*Dermot Gallagher, [Instinct](https://instinct-ware.com). Updated September 19, 2026 - the 31-week mark. The original edition covered the first 8 weeks; this edition extends through 31 weeks and adds the workflow changes that, as much as anything, drove the later work. Part of the [Ceili engine](https://github.com/instinct-ware/ceili) project.*
...
Thirty-one weeks in, a line counter reports **410,060 hand-authored lines** across 1,725 files and more than two dozen packages - C++, C#, Lua, the material and entity authoring DSLs, and HLSL - with blanks, comments, and generated code excluded, alongside about 91,000 lines of session prompt and design notes. It ships with its own embedded Claude agent, reachable over MCP, that can drive the engine's scripting API; it has a real-time multiplayer layer backed by a real-UDP transport; it has a full plane-set brush editor with CSG; it renders skinned meshes with skeletal animation driven by an animation state machine authored as an asset; it lights through a deferred G-buffer and a clustered light grid with cascaded sun shadows; and it authors materials through a dual-model pipeline that carries a physically based and a legacy shading model side by side. It has physics, audio, and particle subsystems. It is built primarily by one developer with Claude Code as a regular collaborator.
...
## 31-Week Results: The Numbers

Ceili began using Claude Code on February 14th, 2026. The first edition of this post covered the 8 weeks through April 12th. This edition extends the count through September 19th - the 31-week mark.
...
| Aug 13 - Sep 19 | 38 | 227,275 | ~41,866/wk |
| **Total (Feb 14 - Sep 19)** | **218** | **1,163,115** | **~37,354/wk avg** |
...
The seventh block (Aug 13 - Sep 19) settles back to about 42k a week, which is
roughly the fifth block's rate and less than half the sixth's. The sixth block
was inflated by two one-time sweeps, and this one has no equivalent: the largest
single commit in it touches 6,165 lines and is real work on the texture derive.
So the run's honest shape is a rising curve through five blocks, one outlier
driven by sweeps, and a settled rate in the low forties. Measured against the
stronger pre-Claude baseline that is about 23x.

### Sessions

Session transcripts older than about a month rotate off disk, so a clean 31-week count isn't recoverable. The window still on disk is 33 days (August 17 to September 19) and holds **365 sessions** in the engine's main project directory, roughly eleven a day, against 192 over a comparable window a month earlier. Sessions skew heavily toward the large end: the multi-hour architecture and implementation sessions account for most of the output, while a long tail of tiny transcripts represents quick probes and questions that never grew into a full working session. Part of the rise is genuine pace and part is a change in shape, with more work split across parallel worktree sessions than before.

### Commits

**6,850 commits** since February 14th. The first 8 weeks produced 423; the most recent block produced **1,814**. The small-commit discipline held throughout: build-and-verify after each language boundary keeps changes bisectable and individually meaningful, and it is what makes a mined defect record possible. Reading back a month of fixes and sorting them by shape, which is where three of this block's Core additions came from, only works when each fix is one commit with one cause.
...
### What Got Built: Weeks 27-31 (Aug 13 - Sep 19)

Five weeks that were almost entirely graphics, plus a small set of Core additions
that came out of reading the block's own defects back.

#### Shadows, from nothing (Aug 26 onward)

The block opened on a finding: the engine rendered no cast shadows at all. What
landed is a four-cascade directional shadow map in a per-scene 2x2 atlas, built in
phases with a measurement at each one. Phase 1 deliberately rendered nothing, so
every frame stayed bit-identical while the atlas, the cascade maths and the cull
plumbing went in.

Three things from it are worth carrying. The cascade frustums ride along with the
camera cull rather than running four extra passes over the scene. The instanced
bake sibling was worth 9.7x on the boids scene, which had gone from 150 fps to 5
because a coalesced batch of N casters cost N draws per cascade. And a caching
mode that re-bakes a cascade only when its fit moved or its content changed
measured the bake at 0.383 ms live against 0.030 ms cached.

#### The deferred renderer, and one line that cost four sessions

A hybrid deferred path went on by default on September 8th: a batch whose material
carries an eligible G-buffer pass is written once and lit from the screen, and
everything else stays forward lit in the same view.

What held it was one line. The deferred lighting material handed the shadow
function a `float3(0, 0, cos)` as a tangent-space light vector, and that function
reads the cosine as `normalize(v).z`, which is 1 for every positive cosine. So the
normal-offset shadow bias was inert across the whole deferred path, and every
deferred-lit surface self-shadowed about 12 percent dark.

The diagnosis went wrong twice before it went right, and both wrong turns are
instructive. It was read as an ambient gap, which the three ambient paths
disproved by agreeing to 0.007. Then as a lighting-model difference, which the
same quads disproved by agreeing with shadows off. One intermediate reading of 8.7
percent turned out to be the post chain's vignette, measured because two probes
sat at different distances from the frame centre. Two parity gates now hold the
two paths together, with their probes on a calibrated circle.

#### The light grid (Sep 16-18)

A clustered light grid went on by default. A forward-lit batch draws once through
a pass whose fragment shader loops the lights in its screen cell: 42,570 forward
draws become 3,330 on the ported RedMines scene, for -35 percent of frame GPU time
on Vulkan and -44 percent on D3D12, at 36 of 36 measured pairs.

The same grid was then pointed at the deferred path, and the result was reported
as what it was. Light draws fall from 3,776 to 236 and the lighting pass drops 60
percent, while total frame GPU time moves -2.1 percent and -1.0 percent, at 17 and
19 of 36 pairs. That is a coin flip, so clustered deferred stayed off by default.
The per-light deferred path was already bounded by its scissor and its volume, so
clustering removed draw-call overhead and not shading work.

Reporting a negative result plainly is a practice worth naming. The temptation
with an AI collaborator is to accept the arm that sounds better, and the guard
against it is to fix the measurement protocol first: interleaved rounds, a control
pass the change cannot affect, and a stated count of how many pairs agreed.

#### The GPU memory model

Each backend had been keeping its own frame counter and its own ring of buffers.
The trunk took both: it numbers every frame it records, owns the frames-in-flight
count, and owns a record-time memory ring that the recording thread writes
directly. Dynamic geometry now writes once into that ring instead of staging and
copying.

#### A defect audit that produced four Core primitives

About 270 defect fixes from the previous month were read back and sorted by shape,
and three shapes were common enough to earn a primitive each.

The largest was a path that cannot do its work, takes a safe early out, and
reports success. Thirty-six of the 270. A Vulkan backend dropping an instanced
draw and returning success; a shadow bake dropping 1,402 skinned casters with four
reaching the atlas. Those now declare themselves at the call site, count, and fail
a gate by default.

The mirror shape got the same table: a check that passed because its input was
empty rather than because the thing under test was correct. A managed tier that
invoked no managed code passed vacuously. An A/B pair whose "off" arm was still on
measured the same thing twice and read as "shadows are free". A witness proves a
path ran, and a run that expected one and did not get it fails.

The other two are smaller and just as useful: a scope-exit macro, after a function
with five exits put its instance upload on one of them, and a per-scene state
container that had been written five times in four packages with five copies of
the same constant.

The general lesson is about the audit itself. Mining a month of fixes for shapes
is a task an AI does well and a human rarely gets around to, and it only works if
every fix is one commit with one cause. The small-commit discipline paid for
itself here in a way that was not the reason for adopting it.

#### Bringing old art forward

Studio grew a texture pipeline for legacy specular-gloss art: segment an atlas
into parts by reading its normal map, classify each part with a material grade,
paint coverage with a brush, and derive the metallic-roughness map from all of it.
The pixel path is a port of a Python reference, kept free of file access so each
stage can be checked against the original on an integer fixture.

The port also found a bug in its reference. Parameter classes were merging as
deltas rather than replacing, so a set of files that matched a general "panel"
rule before a specific "corrpanel" rule kept wood's roughness variation on a
painted-metal base and shipped as a near-mirror.

#### Three content packages

Two ported showcases and one reference package landed: the DogFighterX aircraft
port with its legacy templates and effects, the Prospector package with 124
converted models and 161 entity templates, and a RefScenes package carrying Intel
Sponza and a Jungle Ruins reference scene.

Real content is where the graphics work was measured, and it is also where it was
found wanting. Every number in this block comes off a ported scene rather than a
synthetic one.
...
The most recent block added a fourth, and it sharpens the third. A green check is not evidence. Two failure shapes account for a large share of what got through a full suite: a path that could not do its work, took a safe early out and reported success, and a check that passed because its input was empty rather than because the thing under test was correct. Neither shows up as a failure anywhere. The fix was mechanical once the shapes were named, and naming them was itself an AI task: read a month of fixes back, sort by cause, count. Ask for the shapes rather than the list.
